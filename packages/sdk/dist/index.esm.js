...
class AuthenticationError extends NoranekoIDError {
    constructor(message, code = ErrorCode.AUTHENTICATION_FAILED, originalError, details) {
...
class NetworkError extends NoranekoIDError {
    constructor(message, code = ErrorCode.NETWORK_ERROR, originalError, status, statusText, details) {
        super(code, message, originalError, details);
        this.status = status;
...
function decodeJWT(token) {
    try {
        ...
        throw new AuthenticationError('JWTトークンのデコードに失敗しました', ErrorCode.INVALID_TOKEN, error instanceof Error ? error : undefined);
    }
}
/**
 * JWTトークンの有効期限をチェック
...
function parseUrlParams(url) {
    try {
        ...
        throw new AuthenticationError('URLパラメータの解析に失敗しました', ErrorCode.INVALID_STATE, error instanceof Error ? error : undefined);
    }
}
/**
 * OAuth2エラーパラメータをパース
...
function resolveConfig(config) {
    // 必須パラメータの検証
    if (!config.clientId) {
        throw new ConfigurationError('clientId is required', ErrorCode.MISSING_REQUIRED_PARAMETER);
    }
    if (!config.issuer) {
        throw new ConfigurationError('issuer is required', ErrorCode.MISSING_REQUIRED_PARAMETER);
    }
    // デフォルト値の設定
...
class NoranekoID {
    ...
    async handleCallback(url) {
        try {
            ...
            if (!callbackUrl) {
                throw new AuthenticationError('Callback URL is not available', ErrorCode.INVALID_STATE);
            }
            ...
            if (!code) {
                throw new AuthenticationError('Authorization code not found', ErrorCode.AUTHENTICATION_FAILED);
            }
            // Stateの検証
            const storedState = this.getFromStorage('pkce_state');
            if (!storedState || storedState !== returnedState) {
                throw new AuthenticationError('Invalid state parameter', ErrorCode.INVALID_STATE);
            }
            // PKCE Code Verifierを取得
            const codeVerifier = this.getFromStorage('pkce_code_verifier');
            if (!codeVerifier) {
                throw new AuthenticationError('PKCE code verifier not found', ErrorCode.PKCE_ERROR);
            }
            // アクセストークンを取得
    ...
    async refreshTokens() {
        if (!this.state.refreshToken) {
            throw new AuthenticationError('No refresh token available', ErrorCode.TOKEN_REFRESH_FAILED);
        }
        try {
            ...
            const authError = error instanceof AuthenticationError
                ? error
                : new AuthenticationError('Token refresh failed', ErrorCode.TOKEN_REFRESH_FAILED, error instanceof Error ? error : undefined);
    ...
    async fetchUserInfo() {
        if (!this.state.accessToken) {
            throw new AuthenticationError('No access token available', ErrorCode.AUTHENTICATION_FAILED);
        }
...
export { AuthenticationError, ConfigurationError, ErrorCode, NetworkError, NoranekoID, NoranekoIDError, PKCEError, StorageError, UnsupportedBrowserError, base64UrlDecode, base64UrlEncode, checkBrowserSupport, createNetworkError, createOAuth2Error, decodeJWT, NoranekoID as default, generateRandomString, isTokenExpired, parseUrlParams };
